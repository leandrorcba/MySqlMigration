...
SET @id_efectivo := (SELECT id_tipo_pago FROM tipo_pago WHERE tipo = 'PRESUPUESTO');
...
SELECT
  ...
  p.fecha_cobrado
FROM presupuesto p
WHERE p.precio_cobrado IS NOT NULL and p.precio_cobrado != 0;

ALTER TABLE presupuesto
  DROP COLUMN fecha_cobrado,
  DROP COLUMN precio_cobrado;
